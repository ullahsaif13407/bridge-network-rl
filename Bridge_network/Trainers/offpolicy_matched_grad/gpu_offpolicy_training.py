...
class OffPolicyConfig:
    ...
    # ========================================================================
    # Data Collection
    batch_envs: int = 128           # Increased from 32: More parallel envs = faster data collection
    ...
    # Training Efficiency — matched to PPO/GRPO gradient throughput
    # PPO/GRPO: 4 epochs × 4 minibatches × every episode = 16 grad steps/episode
    # Off-policy: grad_steps × (1/update_every) per episode → 16 × 1 = 16 grad steps/episode
    update_every: int = 1         # Update every episode (matches PPO/GRPO cadence)
    mini_batch: int = 1024         # Larger batches = better GPU utilization
    grad_steps: int = 16          # 16 grad steps per update (matches PPO/GRPO: 4 epochs × 4 minibatches)
    ...
    lr_schedule_updates: Optional[int] = None   # if None, uses total_episodes // lr_schedule_step_every ; I want the start of annealing at 50k episode number and end anneling at 1M episode
    lr_schedule_start_episode: int = 100000      # start annealing at 100k episode number
    lr_schedule_end_episode: int = 1000000      # end annealing at 1M episode number
    lr_schedule_min_lr_ratio: float = 0.01      # final_lr = initial_lr * ratio (clamped)
    lr_schedule_step_every: int = 4000           # step scheduler every N update() calls (keeps same per-episode LR trajectory as update_every=4 variant)
    ...
    save_best: bool = True        # Save best model based on average cost
...
class ReplayBuffer:
    ...
    def reset(self):
        ...
        self.behavior_probs = torch.zeros((self.capacity, TOT_COMP, NACOMP), device=self.device)  # Full probs
    ...
    def push(
        ...
        behavior_probs: torch.Tensor,  # Full probability distributions
        costs: torch.Tensor,
        ...
        self.behavior_probs[idx] = behavior_probs
...
class OffPolicyTrainer:
    ...
    def __init__(self, config: OffPolicyConfig):
        ...
        # Optional: fused Adam on CUDA (falls back if unsupported).
    ...
    def _init_lr_schedulers(self) -> None:
        ...
        if total_updates is None:
            num_updates = (self.cfg.lr_schedule_end_episode-self.cfg.lr_schedule_start_episode) // (self.cfg.update_every*self.cfg.lr_schedule_step_every)
    ...
    def collect_rollout(self):
        ...
        for t in range(ep_len):
            ...
            with torch.no_grad():
                ...
                # Store full behavior probabilities (MATCH LEGACY: behavior_ac)
            ...
            # Buffer always stores yt_ratio as tensor (zeros when no budget)
    ...
    def update(self, episode: int):
        ...
        for _ in range(self.cfg.grad_steps):
            ...
            # Get mu(a|s) for taken actions (behavior policy)
            mu_aa = torch.gather(batch['behavior_probs'], 2, actions.unsqueeze(-1)).squeeze(-1)  # (bs, tot_comp)
            
            # Compute ratio with product (MATCH LEGACY), using log for stability
            log_ratio = (torch.log(pi_aa + 1e-10) - torch.log(mu_aa + 1e-10)).sum(dim=1)
            ...
            log_probs = torch.log(pi_aa + 1e-10)  # (bs, tot_comp)
            # Sum over components, then mean over batch
            actor_loss = -(log_probs * weighted_adv.unsqueeze(1).detach()).sum(dim=1).mean()
        ...
        if self.actor_scheduler is not None and (self.update_calls % step_every == 0 and self.cfg.lr_schedule_start_episode < episode < self.cfg.lr_schedule_end_episode):
            self.actor_scheduler.step()
        if self.critic_scheduler is not None and (self.update_calls % step_every == 0 and self.cfg.lr_schedule_start_episode < episode < self.cfg.lr_schedule_end_episode):
